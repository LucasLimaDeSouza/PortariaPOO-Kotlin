...
class Portaria {
    ...
    private fun controle(): String {
        ...
        if (!convidadoBusiness.maiorDeIdade(convidado.idade)) return "Negado. Menores de idade não são permitidos."


        convidado.tipo = Console.readString("Qual é o tipo de convite? ")
        if (!convidadoBusiness.tipoValido(convidado.tipo)) return "Negado. Convite inválido."


        convidado.codigo = Console.readString("Qual o código do convite? ")
        if (!convidadoBusiness.conviteValido(convidado)) return "Negado. Convite inválido."
